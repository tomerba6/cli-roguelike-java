...
public class EnemyFactory {
    ...
    public static Enemy createEnemy(char tile) {
        return switch (tile) {
            // --- Monsters ---
            case 's' -> new Monster("Gold Cloak", 80, 8, 3, 25, 3, 's');
            case 'k' -> new Monster("Knight", 200, 14, 8, 50, 4, 'k');
            case 'q' -> new Monster("Queen's Guard", 400, 20, 15, 100, 5, 'q');
            case 'z' -> new Monster("Wright", 600, 30, 15, 100, 3, 'z');
            case 'b' -> new Monster("Bear", 1000, 75, 30, 250, 4, 'b');
            case 'g' -> new Monster("Giant", 1500, 100, 40, 500, 5, 'g');
            case 'w' -> new Monster("White Walker", 2000, 150, 50, 1000, 6, 'w');

            // --- Bosses (Upgraded Monsters) ---
            case 'M' -> new Boss("The Mountain", 1000, 60, 25, 500, 6, 5, 'M');
            case 'C' -> new Boss("Queen Cersei", 100, 10, 10, 1000, 1, 8, 'C');
            case 'K' -> new Boss("Night's King", 5000, 300, 150, 5000, 8, 3, 'K');

            // --- Traps ---
            case 'B' -> new Trap("Bonus Trap", 1, 1, 1, 250, 1, 5, 'B');
